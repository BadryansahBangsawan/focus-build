...
Thanks for improving Focus Build.

End users install with `brew tap BadryansahBangsawan/mac-menu-apps` and `brew install --cask focus-build`. This file is for source contributors.
...
Do not commit `dist/`, `.build/`, `.swiftpm/`, or secrets. Do not run `dist/` next to `/Applications/FocusBuild.app` (same bundle ID).
